...
import { Flame, Play, Clock, Activity } from 'lucide-react';
    ...
    <div className="space-y-12 pb-20 max-w-2xl mx-auto px-4 relative">
      ...
      {heroManga && (
        <section className="relative w-full aspect-[4/5] rounded-[3rem] overflow-hidden group border border-white/5 bg-neutral-900 shadow-2xl animate-in fade-in zoom-in-95 duration-1000">
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-8 sm:p-10 space-y-5">
            ...
            <h1 className="text-2xl font-black uppercase tracking-tighter leading-tight text-white drop-shadow-2xl">
              {getMangaTitle(heroManga)}
            </h1>
            <p className="text-[10px] text-neutral-400 font-medium line-clamp-2 leading-relaxed max-w-xs opacity-80">
              {heroManga.attributes.description.en || "Synchronizing data summary..."}
            </p>
            <Link 
              href={`/series/${heroManga.id}`}
              className="inline-flex items-center gap-2 px-10 py-4 bg-white text-black font-black rounded-2xl text-[9px] uppercase tracking-widest hover:bg-accent hover:text-white transition-all duration-500 shadow-xl w-fit active:scale-95"
            >
              <Play className="w-3.5 h-3.5 fill-current" /> Initialize Link
            </Link>
          </div>
        </section>
      )}
      ...
      <section className="space-y-8">
        <div className="flex items-center justify-between px-2 animate-in fade-in slide-in-from-bottom duration-700 delay-500">
          <div className="space-y-1">
            <h2 className="text-[12px] font-black uppercase tracking-tighter text-white">Latest Uploads</h2>
        ...
        {error ? (
          <div className="py-20 text-center space-y-4 bg-neutral-900/50 rounded-[2rem] border border-dashed border-white/5 animate-in fade-in">
            ...
            <Link href="/" className="inline-block px-8 py-3 bg-accent text-white rounded-xl text-[8px] font-black uppercase tracking-widest shadow-lg shadow-accent/20">Resync Node</Link>
          </div>
        ) : (
              ...
              >
                <div className="aspect-[2/3] relative rounded-[2rem] overflow-hidden border border-white/5 bg-neutral-900 shadow-lg group-hover:shadow-accent/10 transition-all duration-700">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-4">
                    <span className="text-[8px] font-black uppercase text-accent tracking-widest drop-shadow-md">View Node</span>
                  </div>
                ...
                <div className="px-1 space-y-1.5">
                  <h3 className="text-[10px] font-black uppercase tracking-tight text-white line-clamp-1 group-hover:text-accent transition-colors duration-300">
                    {getMangaTitle(manga)}
                  </h3>
                  <div className="flex items-center gap-3">
                    <span className="text-[8px] font-black text-neutral-600 uppercase tracking-widest flex items-center gap-1.5">
