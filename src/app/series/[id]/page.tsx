...
export default async function SeriesPage({ params }: SeriesPageProps) {
  ...
  try {
    const [mangaRes, chaptersRes] = await Promise.all([
      ...
      fetch(`https://api.mangadex.org/manga/${id}/feed?translatedLanguage[]=en&order[chapter]=desc&limit=100`)
    ]);

    if (!mangaRes.ok || !chaptersRes.ok) {
      throw new Error(`API error: Manga ${mangaRes.status}, Chapters ${chaptersRes.status}`);
    }
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="w-full md:w-64 flex-shrink-0 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Link href={chapters && chapters.length > 0 ? `/reader/${chapters[0].id}` : '#'} className="flex items-center justify-center gap-2 py-3 bg-white text-black rounded-xl font-black text-[9px] uppercase tracking-widest shadow-xl">
        ...
        <section className="space-y-6 pt-10 border-t border-white/5">
          <div className="space-y-1">
            <h2 className="text-lg font-black tracking-tighter flex items-center gap-2 uppercase">
              <List className="w-5 h-5 text-accent" /> Chapters
            </h2>
            <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest opacity-60">{chapters?.length || 0} Releases found</p>
          </div>

          <div className="grid gap-2">
            {!chapters || chapters.length === 0 ? (
              <div className="text-center py-16 glass rounded-2xl border-dashed">
                <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">No chapters available</p>
              </div>
            ) : (
              chapters.map((chapter: any) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-black text-[11px] tracking-tight uppercase">Chapter {chapter.attributes.chapter}</h4>
    ...
    return (
       <div className="flex flex-col items-center justify-center min-h-[50vh] text-center p-8 space-y-4">
        ...
        <h1 className="text-xl font-black uppercase tracking-tighter">Failed to Load Series</h1>
        <p className="text-xs text-muted-foreground uppercase tracking-widest max-w-xs">{error instanceof Error ? error.message : 'Unknown network error'}</p>
        <Link href="/" className="px-8 py-3 bg-accent text-white rounded-xl font-black uppercase tracking-widest text-[9px]">Return Home</Link>
      </div>
    );
  }
}
