...
import { User, Crown, BookOpen, Quote } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0">
        <div className={cn(
          "w-9 h-9 rounded-2xl flex items-center justify-center border transition-all duration-500 overflow-hidden",
          isOwner ? "border-yellow-500/50 bg-yellow-500/10" : "border-white/5 bg-[#0a0a0f]"
        )}>
          {message.senderPhoto ? (
            <img src={message.senderPhoto} className="w-full h-full object-cover" alt="" />
          ) : (
            <User className={cn("w-4 h-4", isOwner ? "text-yellow-500" : "text-neutral-600")} />
          )}
        </div>
      </div>
