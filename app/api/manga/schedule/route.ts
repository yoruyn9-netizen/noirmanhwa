...
export async function GET() {
  try {
    const response = await fetch('https://api.jikan.moe/v4/manga?order_by=published&sort=asc&status=publishing&limit=50', {
      headers: {
    ...
    const schedule = items.map((item: any) => ({
      id: item.mal_id ? String(item.mal_id) : item.url || `unknown-${Math.random()}`,
      ...
      chapters: typeof item.chapters === 'number' ? item.chapters : item.chapters || 'N/A',
      url: item.url || null
    }));
