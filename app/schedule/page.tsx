...
import { ScheduleItem } from '@/types/manga';
  ...
  const loadSchedule = useCallback(async () => {
    ...
    try {
      ...
      if (!payload.success || !Array.isArray(payload.data)) {
        throw new Error(payload.error || 'Invalid jadwal data received');
      }
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="space-y-2">
            ...
            <p className="max-w-2xl text-sm text-neutral-400 uppercase tracking-[0.3em]">Sinkronisasi otomatis setiap 5 menit dengan sumber jadwal resmi.</p>
          </div>
        ...
        {loading ? (
          <div className="w-full max-w-full overflow-hidden grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {Array.from({ length: 8 }).map((_, idx) => (
              <div key={idx} className="h-72 animate-pulse rounded-3xl bg-white/5 border border-white/5" />
            ))}
          </div>
        ) : error ? (
                ...
                <div className="flex flex-1 flex-col gap-4 p-5">
                  ...
                  <div className="space-y-3 text-[10px] text-neutral-400">
                    <div className="flex items-center justify-between gap-3">
                      <span className="flex items-center gap-2 uppercase tracking-[0.25em]"><CalendarDays className="w-3.5 h-3.5 text-accent" /> Tanggal berikut</span>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <span className="flex items-center gap-2 uppercase tracking-[0.25em]"><Clock3 className="w-3.5 h-3.5 text-accent" /> Chapter terakhir</span>
