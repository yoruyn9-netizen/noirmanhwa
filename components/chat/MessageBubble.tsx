  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        {isOwner && (
          <div className="absolute -top-1 -left-1 bg-yellow-500 rounded-full p-1 shadow-xl animate-bounce">
        ...
        <div className={cn("relative group", isMe ? "text-right" : "text-left")}>
          ...
          {isOwner ? (
            <div className="relative cursor-default">
              {/* Prism Shape Geometry */}
              <div className={cn(
                "relative px-6 py-5 text-[12px] font-bold leading-relaxed text-white shadow-[0_20px_50px_rgba(234,179,8,0.1)] overflow-hidden w-fit",
                "bg-gradient-to-br from-[#1a1a1f] to-[#0a0a0f] border border-yellow-500/30",
                "rounded-[2.5rem]",
                isMe ? "rounded-tr-none" : "rounded-tl-none"
              )}>
                {/* Shimmer Sweep */}
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-yellow-500/5 to-transparent -translate-x-full animate-shimmer-sweep" />
                
                {hasReply && (
                  <div className="mb-4 p-4 bg-black/40 border-l-4 border-yellow-500 rounded-2xl text-left opacity-80 backdrop-blur-md">
                    <p className="text-[9px] font-black text-yellow-500 uppercase tracking-widest">{message.replyToUser}</p>
                    <p className="text-[11px] text-neutral-400 line-clamp-2 italic">{message.replyToText}</p>
                  </div>
                )}
                
                <div className="relative z-10 space-y-3">
                  <p className="tracking-wide">{message.text}</p>
                  <div className="flex items-center gap-3 pt-3 mt-1 border-t border-white/5 opacity-40">
                     <div className="w-1.5 h-1.5 rounded-full bg-yellow-500 animate-pulse shadow-[0_0_8px_rgba(234,179,8,1)]" />
                     <span className="text-[7px] font-black uppercase tracking-[0.4em]">Celestial Protocol Active</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className={cn(
              ...
              isMe 
                ...
                : "bg-[#0a0a0f] border-white/10 text-neutral-300 rounded-tl-none"
            )}>
              {hasReply && (
                <div className={cn(
                  "px-4 py-3 bg-black/30 border-b border-white/5 opacity-80 text-left backdrop-blur-sm",
                  ...
                  <p className="text-[10px] text-neutral-400 line-clamp-1 truncate max-w-[240px] italic">"{message.replyToText}"</p>
                </div>
              )}
